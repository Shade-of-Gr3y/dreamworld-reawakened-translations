...
def GET_top(_query):
    ...
    player_data = utility.read_player_data()["member"]
    ...
    eligible_areas = []
    area_weights   = []
    for area_id, area in utility.area_info.items():
    ...
    area = utility.area_info[dreamland_area_id]
    ...
    def pick_area_pokemon():
        ...
        pkmn_entry = utility.pokemon_info.get(lookup_key)
    ...
    def make_pokemon_encounter(pkmn, pdata, obj_id, obj_pkmn_id, category=0):
        ...
        return {
            "object_id":         obj_id,
            "otoken":            utility.generate_otoken(),
            ...
            "pokemon": {
                ...
                "pokename":    utility.lookup_str("pokemon", pkmn["pokemon_no"]),
                "sex_id":      gender_id,
                "action_type": 1,
                "type1":       utility.lookup_str("type", pkmn["type1"]),
                "type2":       utility.lookup_str("type", pkmn["type2"]),
                "speabi1":     utility.lookup_str("ability", pkmn["speabi1"]),
                "speabi2":     utility.lookup_str("ability", pkmn["speabi2"]),
                "speabi3":     utility.lookup_str("ability", pkmn["speabi3"])
            }
        }
...
def GET_tree_top(_query):
    ...
    for _ in range(count):
        pkmn = utility.get_random_pokemon()
        ...
        pokemon_list.append({
            ...
            "type2":             utility.lookup_str("type", pkmn["type2"]),
            "sex_id":            0,
            "pokekaku":          utility.lookup_str("nature", randint(0, 24)),
            "pokeplace":         "Route 1",
            "ball_name":         utility.lookup_str("ball", 4)
        }
    )
...
def POST_game_clear(_query):
    ...
    if encounter["type"] == "pokemon":
        ...
        reward = {
            "pokemon": {
                ...
                "form_no":        pkmn.get("form_no", 0),
                "sex_id":         gender_id,
                "waza_name_disp": None if "special_moves" not in pkmn else utility.lookup_str("move", choice(pkmn["special_moves"])),
                "waza_count":     4,
                "type1":       utility.lookup_str("type", pkmn["type1"]),
                "type2":       utility.lookup_str("type", pkmn["type2"]),
                "speabi1":     utility.lookup_str("ability", pkmn["speabi1"]),
    ...
    #there is an encounter-with-berry branch missing here
        ...
        reward = {
            ...
            "item": {
                "pokeitem_id":   item_id,
                "pokeitem":      utility.lookup_str("item", item_id),
                "poke_item_num": 1,
            },
            "interior": None,
            "present":  None,
        ...
        utility.chest.add_item(item_id, 1)
